...
const rewardCatalog = [
  {
    id: 'convenience',
    title: '편의점 · 즉시 리프레시',
    description: '퇴근길에 바로 쓰기 좋은 즉시 발급형 쿠폰으로 작은 루틴을 기분 좋게 마무리하세요.',
    meta: '즉시 발급 · 유통기한 30일',
    items: [
      {
        id: 'gs25-night-pack',
        name: 'GS25 야식 리셋팩',
        cost: 450,
        description: '삼각김밥 + 에너지바 + 아메리카노 구성으로 밤샘 준비에 단짠 보상.',
        badge: '즉시 발급',
  ...
  },
  {
    id: 'shopping',
    title: '쇼핑 · 라이프 케어',
    description: '면접 준비에 필요한 스테이셔너리, 티켓, 라이프 케어 리워드를 모았습니다.',
    meta: '프리미엄 큐레이션',
    items: [
      {
        id: 'stationery-kit',
  ...
  },
  {
    id: 'giftcard',
    title: '콘텐츠 · 상품권',
    description: '딥러닝을 도와줄 교육/콘텐츠 상품권부터 범용 디지털 기프트까지.',
    meta: '전자 바우처',
    items: [
      {
        id: 'book-culture-10k',
...
export default function RewardShop() {
  ...
  const handleConfirmRedeem = () => {
    ...
    setModalOpen(false)
    navigate('/rewards/complete', { state: { purchase: response.record, item: selectedReward } })
    ...
    <div className="reward-shop">
      <header className="reward-shop__header">
        <div>
          ...
          <p>
            사진 없이도 정갈한 카드 UI로 각 카테고리를 한눈에 살펴보고, 루틴 보상에 어울리는 리워드를 더 스마트하게
            선택하세요.
          </p>
        </div>
      ...
      <Modal
        ...
        footer={
          <div className="redeem-confirm__actions">
            <button type="button" className="cta-button cta-button--ghost" onClick={handleCloseModal}>
              조금 더 둘러볼게요
            </button>
            <button
              type="button"
      ...
      >
        {selectedReward && (
          <div className="redeem-confirm">
            ...
            {modalError && <p className="redeem-confirm__error">{modalError}</p>}
          </div>
        )}
      </Modal>
    </div>
